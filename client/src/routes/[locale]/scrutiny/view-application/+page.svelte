<script>
  ...
  onMount(async () => {
    // ── Use scrutinyToken instead of adminToken ──
    const scrutinyToken = localStorage.getItem('scrutinyToken')
    ...
    // Set scrutinyToken as accessToken so getViewApplicationData picks it up
    localStorage.setItem('accessToken', scrutinyToken);
    console.log('scrutinyToken set as accessToken:', scrutinyToken?.substring(0, 30) + '...');
...
<svelte:head>
  <title>View Application — Scrutiny</title>
</svelte:head>
  ...
  <header class="bg-white shadow-sm border-b border-gray-200 sticky top-0 z-30">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-3 flex items-center justify-between">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-sm font-semibold text-gray-900">MAMFDC — State Scrutiny View</h1>
  ...
  <div class="w-full px-4 sm:px-6 lg:px-10 py-6 pb-10">
    ...
    {#if isLoading}
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-20 text-center">
        <svg class="animate-spin h-9 w-9 text-green-600 mx-auto mb-4" fill="none" viewBox="0 0 24 24">
    ...
    {:else if appData}
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        ...
        <!-- Banner -->
        <div class="bg-gradient-to-r from-green-700 to-green-900 px-6 sm:px-8 py-5">
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            ...
            <span class="inline-flex items-center gap-2 px-4 py-1.5 bg-green-400 text-green-900 rounded-full text-xs font-bold self-start">
              🏛️ Scrutiny View
            </span>
          </div>
        </div>
        ...
        <div class="border-b-4 border-green-200 bg-gradient-to-br from-green-50 to-white">
          <div class="px-6 sm:px-8 py-2 bg-green-700">
          ...
          <div class="px-6 sm:px-8 py-6 flex flex-col lg:flex-row gap-6 items-start">
            ...
            <div class="flex flex-col items-center gap-2 flex-shrink-0">
              {#if appData.documents?.photo}
                <img src={appData.documents.photo} alt="Photo"
                  class="w-32 h-36 object-cover border-4 border-green-300 rounded-lg shadow-lg"/>
              {:else}
                <div class="w-32 h-36 border-4 border-dashed border-green-300 rounded-lg bg-green-50 flex items-center justify-center">
                  <span class="text-xs text-green-400">No Photo</span>
                </div>
              {/if}
              <span class="text-xs text-green-700 font-bold uppercase">Photo</span>
            </div>

            <!-- Core Info -->
            <div class="flex-1 bg-white rounded-lg border-2 border-green-200 p-5 shadow-sm">
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-3">
                {#each [
                  ...
                  <div class="flex flex-col gap-1">
                    <span class="text-xs font-semibold text-green-700 uppercase tracking-wide">{label}</span>
            ...
            <div class="flex flex-col items-center gap-2 flex-shrink-0">
              {#if appData.documents?.signature}
                <div class="bg-white rounded-lg border-4 border-green-300 shadow-lg p-2">
                  <img src={appData.documents.signature} alt="Signature" class="h-24 w-44 object-contain"/>
                </div>
              {:else}
                <div class="h-24 w-44 border-4 border-dashed border-green-300 rounded-lg bg-green-50 flex items-center justify-center">
                  <span class="text-xs text-green-400">No Signature</span>
                </div>
              {/if}
              <span class="text-xs text-green-700 font-bold uppercase">Signature</span>
            </div>
          </div>
        </div>
        ...
        <div class="border-b border-gray-200">
          <div class="bg-green-700 px-6 sm:px-8 py-2">
          ...
          <div class="border-b border-gray-200">
            ...
            <div class="px-6 sm:px-8 pb-3 grid grid-cols-2 sm:grid-cols-4 gap-x-10">
              {#each [
                ...
                ['Loan Amount', formatCurrency(appData.education.loan_required_amount), 'text-green-700'],
          ...
          <div class="border-b border-gray-200">
            ...
            <div class="px-6 sm:px-8 py-4 space-y-4">
              ...
              {#each appData.collateral.properties || [] as prop, i}
                <div>
                  <p class="text-xs font-bold text-green-700 uppercase mb-2">Property {i + 1}</p>
        ...
        {#if appData.allDocs?.length > 0}
          <div>
            <div class="bg-green-700 px-6 sm:px-8 py-2 flex items-center justify-between">
              <h4 class="text-white font-bold text-xs uppercase tracking-widest">Uploaded Documents</h4>
              <span class="text-green-200 text-xs">{appData.allDocs.length} files</span>
            </div>
            <div class="px-6 sm:px-8 py-4 space-y-2">
              {#each appData.allDocs as doc}
                <div class="flex items-center justify-between py-2 px-3 bg-gray-50 rounded-lg border border-gray-200 hover:bg-green-50 hover:border-green-300 transition-colors">
                  <div class="flex items-center gap-3">
                  ...
                  <a href={doc.file_name} target="_blank" rel="noopener noreferrer"
                    class="px-4 py-1.5 bg-green-600 hover:bg-green-700 text-white text-xs font-semibold rounded-lg transition-colors flex items-center gap-1.5">
